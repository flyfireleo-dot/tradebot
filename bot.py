...
OPTIONAL: GEMINI_MODEL (default: gemini-2.5-flash, swap anytime)
"""
...
UPSTOX_DATA_AVAILABLE = bool(UPSTOX_TOKEN)
...
S = {
    ...
    "manual_chain":None,     # manual option-chain override via /chain
}
...
# ── PHASE 2: OPTION CHAIN (UPSTOX PRIMARY, DHAN BACKUP) ──────────────────
...
def _instrument_key_for(name):
    m = {
        "NIFTY": "NSE_INDEX|Nifty 50",
        "BANKNIFTY": "NSE_INDEX|Nifty Bank",
        "SENSEX": "BSE_INDEX|SENSEX",
    }
    return m.get((name or "NIFTY").upper(), "NSE_INDEX|Nifty 50")

def _manual_chain_for(name):
    mc = S.get("manual_chain")
    if not mc:
        return None
    if mc.get("instrument", "NIFTY") != (name or "NIFTY").upper():
        return None
    return mc

def _normalize_upstox_chain(rows, instrument, expiry):
    if not rows:
        return None
    spot = 0
    strikes = {}
    ...
    for row in rows:
        strike = int(round(float(_pick(row, "strike_price", "strike", default=0))))
        ce = _pick(row, "call_options", default={}) or {}
        pe = _pick(row, "put_options", default={}) or {}
        ce_md = _pick(ce, "market_data", default={}) or {}
        pe_md = _pick(pe, "market_data", default={}) or {}
        ce_g = _pick(ce, "option_greeks", default={}) or {}
        pe_g = _pick(pe, "option_greeks", default={}) or {}

        spot = float(_pick(row, "underlying_spot_price", "spot_price", default=spot) or spot or 0)
        ce_oi = float(_pick(ce_md, "oi", "open_interest", default=0) or 0)
        pe_oi = float(_pick(pe_md, "oi", "open_interest", default=0) or 0)
        ce_bid = float(_pick(ce_md, "bid_price", "best_bid_price", default=0) or 0)
        ce_ask = float(_pick(ce_md, "ask_price", "best_ask_price", default=0) or 0)
        ce_ltp = float(_pick(ce_md, "ltp", "last_price", default=0) or 0)
        pe_ltp = float(_pick(pe_md, "ltp", "last_price", default=0) or 0)

        strikes[strike] = {
            "CE": {
                "ltp": ce_ltp,
                "oi": ce_oi,
                "oi_chg": float(_pick(ce_md, "oi_change", "oi_change_pct", default=0) or 0),
                "iv": float(_pick(ce_g, "iv", "implied_volatility", default=0) or 0),
                "bid": ce_bid,
                "ask": ce_ask,
                "spread": max(ce_ask - ce_bid, 0),
                "delta": float(_pick(ce_g, "delta", default=0) or 0),
                "theta": float(_pick(ce_g, "theta", default=0) or 0),
            },
            "PE": {
                "ltp": pe_ltp,
                "oi": pe_oi,
                "oi_chg": float(_pick(pe_md, "oi_change", "oi_change_pct", default=0) or 0),
                "iv": float(_pick(pe_g, "iv", "implied_volatility", default=0) or 0),
                "bid": float(_pick(pe_md, "bid_price", "best_bid_price", default=0) or 0),
                "ask": float(_pick(pe_md, "ask_price", "best_ask_price", default=0) or 0),
                "spread": max(float(_pick(pe_md, "ask_price", "best_ask_price", default=0) or 0) - float(_pick(pe_md, "bid_price", "best_bid_price", default=0) or 0), 0),
                "delta": float(_pick(pe_g, "delta", default=0) or 0),
                "theta": float(_pick(pe_g, "theta", default=0) or 0),
            },
        }
        total_ce += ce_oi
        total_pe += pe_oi
        if ce_oi > max_ce:
            max_ce = ce_oi; ce_wall = strike
        if pe_oi > max_pe:
            max_pe = pe_oi; pe_wall = strike

    if not strikes:
        return None
    if not spot:
        ordered = sorted(strikes)
        spot = ordered[len(ordered)//2]
    atm = min(strikes.keys(), key=lambda s: abs(s - spot))
    atm_ce = strikes.get(atm, {}).get("CE", {})
    atm_spread = float(atm_ce.get("spread", 0) or 0)
    atm_ltp = float(atm_ce.get("ltp", 0) or 0)
    ...
    return {
        ...
        "ce_wall": int(ce_wall),
        "pe_wall": int(pe_wall),
        "atm_iv": float(atm_ce.get("iv", 0) or 0),
        "exec_quality": round((1 - atm_spread / atm_ltp) * 5, 1) if atm_ltp else 3,
    }

def get_chain_upstox(instrument="NIFTY"):
    if not UPSTOX_DATA_AVAILABLE:
        return None
    try:
        today = datetime.now(IST).date()
        underlying = _instrument_key_for(instrument)
        # try current weekly expiry, then next week
        expiries = []
        for offset in (0, 7):
            base = today + timedelta(days=offset)
            exp = base + timedelta(days=(3 - base.weekday()) % 7)
            expiries.append(exp.strftime("%Y-%m-%d"))
        headers = {"Accept": "application/json", "Authorization": f"Bearer {UPSTOX_TOKEN}"}
        for expiry in expiries:
            r = requests.get("https://api.upstox.com/v2/option/chain", headers=headers, params={"instrument_key": underlying, "expiry_date": expiry}, timeout=12)
            if not r.ok:
                log.warning(f"Upstox chain {instrument} {expiry}: {r.status_code} {r.text[:180]}")
                continue
            data = r.json()
            rows = data.get("data") or []
            norm = _normalize_upstox_chain(rows, instrument, expiry)
            if norm:
                return norm
    except Exception as e:
...
def get_chain_dhan(instrument="NIFTY"):
    if instrument.upper() != "NIFTY" or not P2:
        return None
    try:
        ...
        chain=data.get("data",{});res={"source":"DHAN","instrument":"NIFTY","expiry":exp,"spot":spot,"atm":atm,"strikes":{}}
        ...
        for strike in [atm-100,atm-50,atm,atm+50,atm+100]:
            ...
            res["strikes"][strike]={
                ...
                "PE":{"ltp":pe.get("last_price",0),"oi":pe.get("open_interest",0),
                      ...
                      "bid":pe.get("best_bid_price",0),"ask":pe.get("best_ask_price",0),
                      "spread":pe.get("best_ask_price",0)-pe.get("best_bid_price",0),
...
def get_chain(instrument="NIFTY"):
    manual = _manual_chain_for(instrument)
    if manual:
        return manual
    chain = get_chain_upstox(instrument)
    if chain:
        return chain
    return get_chain_dhan(instrument)
...
/chain PCR CE_WALL PE_WALL [INSTRUMENT] — Manual option chain override
...
"Which stocks are good for swing trade this week?" """,cid)
                    ...
                    chain=d.get("chain")
                    pcr=f"PCR:{chain['pcr']} | " if chain and chain.get('pcr') is not None else ""
...
Based on REAL indicators above + current news — give clear recommendation."""
                    ...
                    kill_reset(); send("✅ Kill switch reset. Bot active again.", cid)

                elif text=="/positions":
                    send("📦 OPEN POSITIONS\n─────────────────────\n" + portfolio_summary(), cid)

                elif text.lower().startswith("/chain "):
                    parts = text.split()
                    if len(parts) >= 4:
                        try:
                            pcr = float(parts[1]); ce_wall = int(float(parts[2])); pe_wall = int(float(parts[3]))
                            instrument = parts[4].upper() if len(parts) > 4 else "NIFTY"
                            S["manual_chain"] = {
                                "source": "MANUAL",
                                "instrument": instrument,
                                "spot": S.get("intraday_nifty") or 0,
                                "atm": round((S.get("intraday_nifty") or ce_wall or pe_wall) / 50) * 50 if instrument == "NIFTY" else 0,
                                "strikes": {},
                                "pcr": pcr,
                                "ce_wall": ce_wall,
                                "pe_wall": pe_wall,
                            ...
                            send(f"✅ Manual chain saved for {instrument}: PCR {pcr}, CE wall {ce_wall}, PE wall {pe_wall}", cid)
                        except Exception:
                            send("Format: /chain PCR CE_WALL PE_WALL [INSTRUMENT]\nExample: /chain 1.08 24500 24200 NIFTY", cid)
                    else:
                        send("Format: /chain PCR CE_WALL PE_WALL [INSTRUMENT]\nExample: /chain 1.08 24500 24200 NIFTY", cid)
...
Chain:{'✅ '+(d.get('chain',{}).get('source','LIVE') if d.get('chain') else '') if d.get('chain') else '⚠️ add UPSTOX/DHAN or use /chain'}
