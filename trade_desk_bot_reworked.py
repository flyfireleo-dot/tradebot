...
def find_support_resistance(closes, highs, lows, lookback=20):
    """Find recent swing support/resistance and 52-week extremes."""
    ...
    return {
        ...
        "52w_high": round(yearly_high, 2),
        "52w_low": round(yearly_low, 2),
    }
...
# ── STATE ─────────────────────────────────────────────────────────────────
USER_CAPITAL       = 500000
MAX_RISK_PER_TRADE = 15000   # user-configured max risk per trade
MAX_DAILY_LOSS     = 45000   # 3x trade risk daily hard stop5% daily hard stop
...
S = {
    ...
    "manual_chain":{},        # manual chain fallback by instrument
    "chain_cache":{},         # live chain cache by instrument
    "last_signal":None,
}
...
INDEX_CONFIG = {
    "NIFTY": {"spot_key": "nifty", "yahoo": "^NSEI", "step": 50, "lot_size": 65, "upstox_key": "NSE_INDEX|Nifty 50", "dhan_scrip": 13},
    "BANKNIFTY": {"spot_key": "banknifty", "yahoo": "^NSEBANK", "step": 100, "lot_size": 35, "upstox_key": "NSE_INDEX|Nifty Bank", "dhan_scrip": None},
    "SENSEX": {"spot_key": "sensex", "yahoo": "^BSESN", "step": 100, "lot_size": 20, "upstox_key": "BSE_INDEX|SENSEX", "dhan_scrip": None},
}
...
def signal_to_text(signal: TradeSignal) -> str:
    if signal.action == "NO_TRADE":
        return (
            "NO TRADE\n"
            f"REASON: {signal.reason}\n"
            "WATCH FOR: Wait for clean trend pullback or opening range breakout\n"
            "LEARNED: Discipline is also a position."
        )
    return (
        f"SIGNAL: {'BULLISH' if signal.option_type == 'CE' else 'BEARISH'}\n"
        f"PLAYBOOK: {signal.playbook}\n"
        f"STRIKE: {signal.instrument} {signal.strike} {signal.option_type} {signal.expiry}\n"
        f"PREMIUM: ₹{signal.premium_estimate if signal.premium_estimate is not None else 0:.0f}\n"
        f"LOTS: {signal.lots}\n"
        f"SL: ₹{signal.stop_loss:.0f}\n"
        f"TARGET: ₹{signal.target_price:.0f}\n"
        f"ENTRY: Next confirmed signal candle close IST\n"
        f"EXIT BY: Same day or earlier if stop/target hits\n"
        f"TOTAL MONEY: ₹{((signal.premium_estimate or 0) * signal.lots * 65):,.0f}\n"
        f"QUALITY: {signal.quality}/20\n"
        f"CONFIDENCE: {signal.confidence}/10\n"
        f"RATIONALE: {signal.reason}\n"
        "LEARNED: Protect capital when structure is unclear."
    )


def calculate_lots(entry_price, stop_loss, lot_size=65, capital=USER_CAPITAL, max_risk_rupees=MAX_RISK_PER_TRADE, max_lots=10):
    if entry_price is None or stop_loss is None:
        return 0
    per_lot_risk = abs(entry_price - stop_loss) * lot_size
    if per_lot_risk <= 0:
        return 0
    allowed_risk = min(max_risk_rupees, capital * 0.03)
    lots = int(allowed_risk // per_lot_risk)
    return max(1, min(lots, max_lots))


def estimate_option_premium(spot_price, atr, option_type):
    if not spot_price or not atr:
        return None
    base = max(80.0, atr * 0.35)
    skew = 1.05 if option_type == "PE" else 1.0
    return round(base * skew, 2)


def trend_pullback_signal(instrument, tech, data, chain=None):
    if not tech:
        return None
    ...
    rsi = tech.get("rsi")
    atr = tech.get("atr")
    vol_ok = tech.get("volume_signal") != "LOW — weak conviction"
    vix = float(data.get("vix", {}).get("price") or 18)
    if not all([price, ema20, ema50, ema200, rsi, atr]) or not vol_ok:
        return None
    if vix > 24:
        return None
    ...
    if chain and chain.get("pcr") is not None:
        ...
        if 0.8 <= pcr <= 1.35:
            chain_boost += 1
            chain_note = f" | chain PCR {pcr} healthy"
        elif pcr < 0.6 or pcr > 1.6:
            chain_boost -= 1
            chain_note = f" | chain PCR {pcr} extreme"

    if price > ema20 > ema50 > ema200 and rsi >= 55:
        entry = price
        stop = round(min(float(ema20), float(tech.get("support", entry))) - atr * 0.5, 2)
        if stop >= entry:
            stop = round(entry - atr * 1.2, 2)
        target = round(entry + (entry - stop) * 2.0, 2)
        return {
            "direction": "BULLISH", "instrument": instrument, "option_type": "CE",
            "entry_price": entry, "stop_loss": stop, "target_price": target,
            "confidence": 7 + max(0, chain_boost), "quality": 15 + max(0, chain_boost),
            "playbook": "PB-01",
            "reason": f"{instrument} trend pullback confirmed by EMA alignment, RSI strength, and acceptable volatility{chain_note}",
        }

    if price < ema20 < ema50 < ema200 and rsi <= 45:
        entry = price
        stop = round(max(float(ema20), float(tech.get("resistance", entry))) + atr * 0.5, 2)
        if stop <= entry:
            stop = round(entry + atr * 1.2, 2)
        target = round(entry - (stop - entry) * 2.0, 2)
        return {
            "direction": "BEARISH", "instrument": instrument, "option_type": "PE",
            "entry_price": entry, "stop_loss": stop, "target_price": target,
            "confidence": 7 + max(0, chain_boost), "quality": 15 + max(0, chain_boost),
            "playbook": "PB-02",
            "reason": f"{instrument} downtrend continuation confirmed by EMA alignment, weak RSI, and acceptable volatility{chain_note}",
        }
    return None


def opening_range_breakout_signal(instrument, data, tech, chain=None):
    cfg = INDEX_CONFIG[instrument]
    spot = float(data.get(cfg["spot_key"], {}).get("price") or 0)
    vix = float(data.get("vix", {}).get("price") or 18)
    if not spot or not tech or vix > 26:
        return None
    pcr = (chain or {}).get("pcr")
    atr = tech.get("atr")
    if not atr:
        return None
    if pcr is None and not chain:
        # advanced fallback: allow price-action breakout without chain, but lower quality
        pcr = 1.0
    elif pcr is None:
        return None
    pcr = float(pcr)
    resistance = float(tech.get("resistance") or spot)
    support = float(tech.get("support") or spot)

    if spot > resistance * 0.998 and 0.7 <= pcr <= 1.5:
        entry = spot
        stop = round(entry - atr * 1.0, 2)
        target = round(entry + atr * 1.8, 2)
        return {
            "direction": "BULLISH", "instrument": instrument, "option_type": "CE",
            "entry_price": entry, "stop_loss": stop, "target_price": target,
            "confidence": 6 if chain else 5, "quality": 14 if chain else 12,
            "playbook": "PB-04",
            "reason": f"{instrument} breakout bias supported by {'live option chain and ' if chain else ''}price near resistance",
        }

    if spot < support * 1.002 and 0.7 <= pcr <= 1.5:
        entry = spot
        stop = round(entry + atr * 1.0, 2)
        target = round(entry - atr * 1.8, 2)
        return {
            ...
            "confidence": 6 if chain else 5, "quality": 14 if chain else 12,
            "playbook": "PB-04",
            "reason": f"{instrument} breakdown bias supported by {'live option chain and ' if chain else ''}price near support",
        }
    return None


def generate_trade_signal(data, capital=USER_CAPITAL):
    candidates = []
    for instrument, cfg in INDEX_CONFIG.items():
        tech = get_full_technicals(cfg["yahoo"])
        if not tech:
            continue
        chain = get_chain(instrument)
        if S.get("day_verdict") == "FLAT":
            continue
        raw_signal = trend_pullback_signal(instrument, tech, data, chain) or opening_range_breakout_signal(instrument, data, tech, chain)
        if not raw_signal:
            continue
        lot_size = int((chain or {}).get("lot_size") or cfg["lot_size"])
        lots = calculate_lots(raw_signal["entry_price"], raw_signal["stop_loss"], lot_size=lot_size, capital=capital, max_risk_rupees=MAX_RISK_PER_TRADE, max_lots=8)
        if lots <= 0:
            continue
        strike = int(round(float(data.get(cfg["spot_key"], {}).get("price") or raw_signal["entry_price"]) / cfg["step"]) * cfg["step"])
        premium = None
        expiry = datetime.now(IST).strftime("%d %b %Y")
        if chain:
            if chain.get("expiry"):
                try:
                    expiry = datetime.strptime(chain["expiry"], "%Y-%m-%d").strftime("%d %b %Y")
                except Exception:
                    pass
            premium = ((chain.get("strikes", {}).get(strike, {}) or {}).get(raw_signal["option_type"], {}) or {}).get("ltp")
        if premium is None:
            premium = estimate_option_premium(float(data.get(cfg["spot_key"], {}).get("price") or raw_signal["entry_price"]), tech.get("atr"), raw_signal["option_type"])
        candidates.append(TradeSignal(
            action="BUY", instrument=instrument, option_type=raw_signal["option_type"], strike=strike, expiry=expiry,
            entry_price=round(raw_signal["entry_price"],2), stop_loss=round(raw_signal["stop_loss"],2), target_price=round(raw_signal["target_price"],2),
            lots=lots, confidence=int(raw_signal["confidence"]), quality=int(raw_signal["quality"] + (1 if chain else 0)), playbook=raw_signal["playbook"],
            reason=raw_signal["reason"] + (" | live chain connected" if chain else " | fallback mode active"), premium_estimate=round(premium,2) if premium is not None else None,
            structure="SINGLE_LEG" if chain else "FALLBACK"
        ))

    if not candidates:
        return TradeSignal(action="NO_TRADE", instrument="NIFTY", option_type="NONE", strike=None, expiry="", entry_price=None, stop_loss=None, target_price=None, lots=0, confidence=0, quality=int(S.get("day_quality") or 0), playbook="NONE", reason="No deterministic setup met the rules across NIFTY/BANKNIFTY/SENSEX")
    candidates.sort(key=lambda s: (s.quality, s.confidence, bool(s.premium_estimate), s.instrument == "NIFTY"), reverse=True)
    return candidates[0]


def validate_signal(signal: TradeSignal, capital=USER_CAPITAL, lot_size=None):
    if signal.action == "NO_TRADE":
        return True, "✅ APPROVED — No trade"
    cfg = INDEX_CONFIG.get(signal.instrument, {"lot_size": 65, "step": 50})
    lot_size = lot_size or cfg["lot_size"]
    if signal.quality < 14 and signal.structure != "FALLBACK":
        return False, f"❌ BLOCKED — Quality too low: {signal.quality}/20"
    if signal.confidence < 5:
        return False, f"❌ BLOCKED — Confidence too low: {signal.confidence}/10"
    if None in (signal.entry_price, signal.stop_loss, signal.target_price):
        return False, "❌ BLOCKED — Missing core prices"
    risk = abs(signal.entry_price - signal.stop_loss)
    reward = abs(signal.target_price - signal.entry_price)
    if risk <= 0:
        return False, "❌ BLOCKED — Invalid stop loss"
    rr = reward / risk
    if rr < 1.5:
        return False, f"❌ BLOCKED — Poor R:R {rr:.2f}"
    total_risk = risk * signal.lots * lot_size
    if total_risk > MAX_RISK_PER_TRADE:
        return False, f"❌ BLOCKED — Risk too high Rs{total_risk:,.0f}"
    step = cfg["step"]
    if signal.strike is not None and signal.strike % step != 0:
        return False, f"❌ BLOCKED — Invalid {signal.instrument} strike step"
    return True, "✅ APPROVED — Deterministic checks passed"
...
def calc_atr_sl(tech, current_price, direction="CE", multiplier=1.5):
    """Calculate ATR-guided stop loss around real swing levels."""
    if not tech or not tech.get("atr"):
        return None, None
    atr = tech["atr"]
    support = tech.get("support", current_price - atr)
    resistance = tech.get("resistance", current_price + atr)

    if direction == "CE":
        atr_sl = round(current_price - (atr * multiplier), 0)
        sl = min(atr_sl, round(support - atr * 0.25, 0))
    else:
        atr_sl = round(current_price + (atr * multiplier), 0)
        sl = max(atr_sl, round(resistance + atr * 0.25, 0))
...
def rule_supervisor(alert_text, nifty_price, capital=500000):
    """Legacy text supervisor retained for compatibility, but trades now use object validation."""
    if "NO TRADE" in alert_text:
        return "✅ APPROVED — No trade", "No trade", False
    return "⚠️ CAUTION — Legacy text supervisor should not be used for live trade validation", "legacy", False
...
# ── PHASE 2: OPTION CHAIN (UPSTOX PRIMARY, DHAN BACKUP, MANUAL FALLBACK) ──────────────────
def next_weekly_expiry():
    today = datetime.now(IST).date()
    target = today + timedelta(days=(3 - today.weekday()) % 7)
...
def normalize_chain(raw_rows, instrument, expiry, source="UPSTOX"):
    cfg = INDEX_CONFIG[instrument]
    step = cfg["step"]
    spot = None
    strikes = {}
    ...
    for row in raw_rows or []:
        ...
        if strike is None:
            continue
        strike = int(round(float(strike) / step) * step)
        spot = row.get("underlying_spot_price") or row.get("spot_price") or row.get("underlyingSpotPrice") or spot
        ce = row.get("call_options") or row.get("call") or row.get("CE") or {}
        pe = row.get("put_options") or row.get("put") or row.get("PE") or {}
        ce_ltp = ce.get("market_data", {}).get("ltp") or ce.get("ltp") or ce.get("last_price") or 0
        pe_ltp = pe.get("market_data", {}).get("ltp") or pe.get("ltp") or pe.get("last_price") or 0
        ce_oi = ce.get("market_data", {}).get("oi") or ce.get("oi") or ce.get("open_interest") or 0
        pe_oi = pe.get("market_data", {}).get("oi") or pe.get("oi") or pe.get("open_interest") or 0
        ce_iv = ce.get("option_greeks", {}).get("iv") or ce.get("iv") or ce.get("implied_volatility") or 0
        pe_iv = pe.get("option_greeks", {}).get("iv") or pe.get("iv") or pe.get("implied_volatility") or 0
        strikes[strike] = {
            "CE": {"ltp": float(ce_ltp or 0), "oi": float(ce_oi or 0), "iv": float(ce_iv or 0), "delta": ce.get("option_greeks", {}).get("delta") or ce.get("delta") or 0, "theta": ce.get("option_greeks", {}).get("theta") or ce.get("theta") or 0},
            "PE": {"ltp": float(pe_ltp or 0), "oi": float(pe_oi or 0), "iv": float(pe_iv or 0), "delta": pe.get("option_greeks", {}).get("delta") or pe.get("delta") or 0, "theta": pe.get("option_greeks", {}).get("theta") or pe.get("theta") or 0},
        }
        total_ce += float(ce_oi or 0); total_pe += float(pe_oi or 0)
        if float(ce_oi or 0) > max_ce: max_ce, ce_wall = float(ce_oi or 0), strike
        if float(pe_oi or 0) > max_pe: max_pe, pe_wall = float(pe_oi or 0), strike
    if not strikes:
        return None
    spot = float(spot or sorted(strikes)[len(strikes)//2])
    atm = int(round(spot / step) * step)
    atm_block = strikes.get(atm, {}).get("CE", {})
    return {
        "source": source, "instrument": instrument, "expiry": expiry, "spot": round(spot, 2), "atm": atm,
        "strikes": strikes, "pcr": round(total_pe / total_ce, 2) if total_ce else None,
        "ce_wall": ce_wall, "pe_wall": pe_wall, "atm_iv": atm_block.get("iv", 0),
        "exec_quality": 4.0 if source == "UPSTOX" else 3.5, "lot_size": cfg["lot_size"],
    }


def get_chain_upstox(instrument):
    if not UPSTOX_TOKEN:
        return None
    cfg = INDEX_CONFIG[instrument]
    expiry = next_weekly_expiry()
    instrument_key = cfg["upstox_key"]
    try:
        r = requests.get("https://api.upstox.com/v2/option/chain", headers=upstox_headers(), params={"instrument_key": instrument_key, "expiry_date": expiry}, timeout=15)
        if r.ok:
            data = r.json().get("data") or []
            chain = normalize_chain(data, instrument, expiry, source="UPSTOX")
            if chain:
                return chain
    except Exception as e:
        log.warning(f"Upstox chain {instrument}: {e}")
    return None


def get_chain_dhan(instrument):
    if not P2:
        return None
    cfg = INDEX_CONFIG[instrument]
    if not cfg["dhan_scrip"]:
        return None
    try:
        exp = next_weekly_expiry()
        h = {"access-token": DHAN_TOKEN, "client-id": DHAN_CLIENT, "Content-Type": "application/json"}
        r = requests.post("https://api.dhan.co/v2/optionchain", headers=h, json={"UnderlyingScrip": cfg["dhan_scrip"], "UnderlyingSeg": "IDX_I", "Expiry": exp}, timeout=10)
        payload = r.json()
        raw = []
        for k, v in (payload.get("data") or {}).items():
            if not isinstance(v, dict) or "_" not in k:
                continue
            strike, side = k.split("_", 1)
            strike = int(float(strike))
            found = next((x for x in raw if int(x["strike_price"]) == strike), None)
            if not found:
                found = {"strike_price": strike, "underlying_spot_price": payload.get("last_price") or payload.get("spot_price") or 0, "call_options": {}, "put_options": {}}
                raw.append(found)
            block = {"last_price": v.get("last_price", 0), "open_interest": v.get("open_interest", 0), "implied_volatility": v.get("implied_volatility", 0), "delta": v.get("delta", 0), "theta": v.get("theta", 0)}
            if side.upper() == "CE":
                found["call_options"] = block
            else:
                found["put_options"] = block
        chain = normalize_chain(raw, instrument, exp, source="DHAN")
        if chain:
            return chain
    except Exception as e:
        log.warning(f"Dhan chain {instrument}: {e}")
    return None


def get_chain_manual(instrument):
    manual = S.get("manual_chain", {}).get(instrument)
    if not manual:
        return None
    cfg = INDEX_CONFIG[instrument]
    spot = S.get(f"intraday_{cfg['spot_key']}") or 0
    atm = int(round(float(spot or 0) / cfg["step"]) * cfg["step"]) if spot else 0
    expiry = next_weekly_expiry()
    strikes = {}
    for strike in [atm-cfg["step"], atm, atm+cfg["step"]] if atm else []:
        strikes[strike] = {"CE": {"ltp": 0, "oi": manual.get("ce_wall") == strike and 1 or 0, "iv": 0, "delta": 0, "theta": 0}, "PE": {"ltp": 0, "oi": manual.get("pe_wall") == strike and 1 or 0, "iv": 0, "delta": 0, "theta": 0}}
    return {"source": "MANUAL", "instrument": instrument, "expiry": expiry, "spot": float(spot or 0), "atm": atm or manual.get("ce_wall") or manual.get("pe_wall"), "strikes": strikes, "pcr": manual.get("pcr"), "ce_wall": manual.get("ce_wall"), "pe_wall": manual.get("pe_wall"), "atm_iv": None, "exec_quality": 2.0, "lot_size": cfg["lot_size"]}


def get_chain(instrument="NIFTY"):
    cache = S.get("chain_cache", {}).get(instrument)
    if cache:
        return cache
    chain = get_chain_upstox(instrument) or get_chain_dhan(instrument) or get_chain_manual(instrument)
    if chain:
        S.setdefault("chain_cache", {})[instrument] = chain
    return chain
...
def portfolio_summary():
    ...
    for p in S["positions"]:
        ...
        pnl=(p["current"]-p["entry"])*p["lots"]*lot_size;total_pnl+=pnl
        lines.append(f"{p.get('instrument','NIFTY')} {p['strike']} {p['type']} ×{p['lots']} | P&L: {'+' if pnl>=0 else ''}₹{pnl:,.0f}")
...
def get_gift_nifty():
    """Return reliable GIFT Nifty only. No synthetic or guessed fallback."""
    try:
        r = requests.get(
...
def fetch_data():
    ...
    if gift:
        ...
        d["gift"] = None
        log.info("GIFT Nifty unavailable — no synthetic fallback will be used")
    ...
    S["chain_cache"] = {}
    d["chains"] = {name: get_chain(name) for name in INDEX_CONFIG.keys()}
    d["chain"] = d["chains"].get("NIFTY") or next((c for c in d["chains"].values() if c), None)
...
MAIN_PROMPT="""Reserved for research and explanatory tasks only. Do not use this prompt to create live trade signals."""

SUPERVISOR_PROMPT="""Reserved for commentary only. Live trade validation must use validate_signal()."""
...
def run_alert(atype):
    ...
    try:
        ...
        approved, verdict = validate_signal(signal)
        alert = signal_to_text(signal)

        if not approved:
            S["blocked"] += 1
            send(f"{alert}\n\n{verdict}\n\n❌ DO NOT TRADE — deterministic checks failed.")
            return

        S["blocked"] = 0
        S["last_alert"] = alert
        S["last_alert_type"] = atype
        S["last_signal"] = asdict(signal)

        day_ctx = f"Day quality: {S.get('day_quality')}/20 ({S.get('day_verdict')})"
        state_ctx = f"State: {state} | Threshold: {thresh}/10 ({thresh_note})"
        final_message = f"{alert}\n\n{verdict}\n{day_ctx}\n{state_ctx}"

        is_trade = signal.action != "NO_TRADE"
        buttons = (
            [
                [{"text": "✅ Executed", "data": "btn_executed"}, {"text": "⏭ Skip", "data": "btn_skip"}],
                [{"text": "❓ Why", "data": "btn_why"}, {"text": "📊 Deep", "data": "btn_deep"}],
            ]
            if is_trade
            else [[{"text": "🔔 Alert if setup forms", "data": "btn_watch"}]]
        )
        send(final_message, buttons=buttons)
        ...
        if is_trade:
            wtrade({
                ...
                "state": state,
                "pb": signal.playbook,
                "bias": "BULLISH" if signal.option_type == "CE" else "BEARISH",
                "strike": f"{signal.instrument} {signal.strike} {signal.option_type}",
                "prem": f"₹{signal.premium_estimate or 0:.0f}",
                "lots": signal.lots,
                "risk": f"₹{abs(signal.entry_price - signal.stop_loss) * signal.lots * 65:,.0f}",
                "conf": signal.confidence,
                "qual": signal.quality,
            })
        else:
            wnotrade(signal.reason)
        log.info(f"=== {atype} DONE ===")
    except Exception as e:
        log.error(f"Alert:{e}")
        send(f"⚠️ Error @ {now_str}\n{e}")
...
Chain:{'✅' if d.get('chain') else '⚠️ add UPSTOX vars or use /chain'}
